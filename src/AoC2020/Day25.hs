...
aoc202025 input = 
    let [cardPublicKey, doorPublicKey] = input |> lines |> map readInt
        ...
        --encryptionKeySame = repeatTrans cardPublicKey doorLoopSize False
    in encryptionKey
        
repeatTrans privateKey publicKey isLoopSize = go 1 0 where

    go value loopSize = let
        chkValue = if isLoopSize then value else loopSize
        retValue = if isLoopSize then loopSize else value
        in if chkValue == publicKey
           then retValue
           else let !value' = (privateKey * value) `mod` 20201227
                    !loopSize' = loopSize + 1
