...
import Data.List (sort, group, sortBy, groupBy, nub, intercalate, find)
...
main = do
    ...
    let inputPerTile = input |> lines |> split [""]
        ...
        tileFrames = tiles
                |> map (\(tileid, frame) -> (tileid, frame
                       |> map (\(fingerprint, flipped) -> M.lookup fingerprint groups
                                                          |> maybe 0 id
        ...
        findNextTile getEdge checkPos tile@(tileId, [n, e, s, w]) = case getEdge tile of
                Border -> Nothing
                (Paired fp _) -> (M.lookup fp tileIdsByFp)
...
aoc202020 input = (part1, "MISSING") where
