...
aoc202116 input = (part1, part2) where
    ...
    decodeOp ver typ (b1:bits) =
        let
            lengthTyp   = (if b1 == '0' then 15 else 11)
            ...
            (restbits, subpackets) = case lengthTyp of
                ...
                11 -> let (restBs, _, restPcs) = (bitsPackets, lengthBits, [])
                                                |> until (\(_, counter, _) -> counter == 0)
