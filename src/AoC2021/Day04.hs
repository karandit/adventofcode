...
parseBoard rows = rows
                |> zip [0..]
                |> map (\(rowIdx,row) ->
                    row
                    |> SS.lstrip |> SS.replace "  " " "
                    |> LL.split " "
                    |> zip [0..]
...
findBingo (board:boards) nr newBoards newBingos =
       let
           foundCoord = board |> M.toList |> find (\(_, (v, _)) -> nr == v)
        in case foundCoord of
            Nothing -> findBingo boards nr (board:newBoards) newBingos
            Just ((row,col),_) -> let board' = M.adjust (\(v, _) -> (v, True)) (row, col) board
                                      isBingo = checkBingo (row, col) board'
                                  in if isBingo then findBingo boards nr newBoards ((bingoScore board' nr):newBingos)
...
aoc202104 input = (part1, part2) where
