...
aoc202125 input = part1 where
    ...
    stepCucumbers cucumbers = let
            (cntMovesEast, cucumbers') = cucumbers
                    |> M.foldrWithKey (\(x,y) c acc@(cnt, accM) ->
                            let newPos = ((x+1) `mod` maxX, y)
                            in if c=='>'
                            then isFreePos newPos cucumbers
                                |>(\isFree -> if isFree then (cnt+1, M.insert newPos c accM) else (cnt, M.insert (x,y) c accM))
                            else acc)
                        (0, M.empty)
            (cntMovesSouth, cucumbers'') = cucumbers
                    |> M.foldrWithKey (\(x,y) c acc@(cnt, accM) ->
                            let newPos = (x, (y+1) `mod` maxY)
                            in if c=='v'
                               then isFreePosV newPos cucumbers && isFreePos newPos cucumbers'
                               |>(\isFree -> if isFree then (cnt+1, M.insert newPos c accM) else (cnt, M.insert (x,y) c accM))
                               else acc)
                        (0, cucumbers')
