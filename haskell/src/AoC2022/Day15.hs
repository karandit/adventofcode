...
aoc202215 inputY input = (part1, part2) where
    parseSensor s = s |> split ":" |> map words
        |> (\[["Sensor", "at", 'x':'=':sx, 'y':'=':sy],
              ["closest", "beacon", "is", "at", 'x':'=':bx, 'y':'=':by]] ->
            ((readInt $ init sx, readInt sy), (readInt $ init bx, readInt by)))
    ...
    (foundSensor@(sx,sy), _, foundRad) = diamonds
                       |> filter (\(s1, _, m1) -> diamonds |> find (\(s2, _, m2) -> s1 /= s2 && manhattan s1 s2 == 2 + m1 + m2) |> Maybe.isJust)
